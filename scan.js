...
// 5) Email (SMTP)
async function sendReportEmail(filePath, total) {
  ...
  await transporter.sendMail({
    ...
    to,
    subject: `Metafields pendientes (${total})`,
    text: [
      "Hola,",
      "",
      "Adjunto Excel con productos (Activos y publicados), colecciones (Publicadas) y páginas (Visibles) que no tienen los metafields requeridos.",
...
export default async function scan() {
  ...
  await sendReportEmail(excelPath, rows.length);
